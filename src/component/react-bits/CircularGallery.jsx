...
const DEFAULT_FONT_FAMILY = 'Figtree';
...
function getFontFamily(font) {
  ...
  return tokens.find(token => !/^(bold|normal|italic|oblique|inherit|initial|revert|unset|\d+px|\d+em)$/i.test(token)) || null;
}

function shouldLoadDefaultFont(font) {
  const family = getFontFamily(font);
  if (!family) return false;
  return family.toLowerCase() === DEFAULT_FONT_FAMILY.toLowerCase();
}
...
async function resolveFont(font, fontUrl) {
  const normalizedFont = font?.trim() || DEFAULT_FONT;
  const normalizedFontUrl = fontUrl?.trim();

  // Use the bundled Figtree stylesheet when the caller relies on the default
  // font family, otherwise honor the explicit `fontUrl`.
  const effectiveUrl = normalizedFontUrl || (shouldLoadDefaultFont(normalizedFont) ? DEFAULT_FONT_URL : null);
  if (!effectiveUrl) {
    ...
    if (document.fonts && document.fonts.load) {
      try {
        await document.fonts.load(normalizedFont);
  ...
  try {
    ...
    const sizeMatch = normalizedFont.match(/^\s*(.*?\d+px)/);
    ...
    console.error('CircularGallery: unable to load font from', normalizedFontUrl || fontUrl, error);
    return normalizedFont;
  }
}
...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  const textHeight = Math.ceil(fontSize * 1.45 + paddingY * 2);
...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = '30px sans-serif', container, viewport }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
  ...
  createLabel() {
    if (!this.text || !this.container) return;
    ...
    this.element.style.color = this.textColor || '#ffffff';
    this.element.style.font = this.font;
    this.element.style.fontWeight = '700';
    this.element.style.whiteSpace = 'nowrap';
    this.element.style.pointerEvents = 'none';
    this.element.style.zIndex = '10';
    this.element.style.boxShadow = '0 8px 20px rgba(0, 0, 0, 0.3)';
    this.container.appendChild(this.element);
    this.updateLayout();
  }

  updateLayout() {
    if (!this.element || !this.container || !this.plane) return;

    const rect = this.container.getBoundingClientRect();
    if (!rect.width || !rect.height) return;

    const worldWidth = this.viewport?.width || 1;
    const worldHeight = this.viewport?.height || 1;
    const x = rect.width * (0.5 + this.plane.position.x / worldWidth);
    const planeHeightPx = (this.plane.scale.y / worldHeight) * rect.height;
    const y = rect.height / 2 - (this.plane.position.y / worldHeight) * rect.height / 2 + planeHeightPx / 2 + 18;
...
class Media {
  ...
  update(scroll, direction) {
    ...
    if (this.title && typeof this.title.updateLayout === 'function') {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    if (this.title && typeof this.title.onResize === 'function') {
      this.title.onResize();
    }
  }
}
